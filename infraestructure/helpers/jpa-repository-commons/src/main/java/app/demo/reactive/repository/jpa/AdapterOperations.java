package app.demo.reactive.repository.jpa;
...
public abstract class AdapterOperations<E, D, I, R extends CrudRepository<D, I> & QueryByExampleExecutor<D>> {
    ...
    protected Flux<E> saveAllEntities(Flux<E> entities) {
        return entities.map(this::toData).collectList().flatMapMany(this::saveData).map(this::toEntity);
    }
    ...
    public Mono<E> findById(I id) {
        return doQuery(fromSupplier(() -> repository.findById(id)).subscribeOn(Schedulers.elastic())
    ...
    protected Flux<E> doQueryMany(Supplier<Iterable<D>> query) {
        return fromSupplier(query).subscribeOn(Schedulers.elastic()).flatMapMany(Flux::fromIterable)
                .map(this::toEntity);
    }
    ...
    protected Mono<D> saveData(D data) {
        return fromSupplier(() -> repository.save(data)).subscribeOn(Schedulers.elastic());
    }

    protected Flux<D> saveData(Iterable<D> data) {
        return defer(() -> fromIterable(repository.saveAll(data))).subscribeOn(Schedulers.elastic());
    }
